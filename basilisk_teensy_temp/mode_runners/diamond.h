...
void ExecFuncs::Diamond(Basilisk& b) {
  ...
  switch (fsm) {
    ...
    case FSM::Step: {
      ...
      // Command both phis to target.
      Serial.print(F("Command both phis to target: "));
      Serial.println(c.tgt_phi(c.current_step));
      b.CommandBoth(
          [&](Servo& s) { s.Position(c.tgt_phi(c.current_step)); });
      b.Print();
